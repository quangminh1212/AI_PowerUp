...
pub async fn mix_config_messages(
    ...
    let all_integrations =
        crate::integrations::setting_up_integrations::integrations_all(gcx.clone(), false).await;
    for ig in all_integrations.integrations {
        ...
        let file_content = match fs::read_to_string(&ig.integr_config_path) {
            ...
            Err(err) => {
                tracing::error!(
                    "Failed to read file for integration {}: {:?}",
                    ig.integr_config_path,
                    err
                );
                continue;
            }
    ...
    let mut active_project_path = if current_config_path.starts_with(&global_config_dir) {
        ...
        current_config_path.parent().and_then(|p| {
            p.parent()
                .filter(|gp| {
                    p.file_name() == Some("integrations.d".as_ref())
                        && gp.file_name() == Some(".refact".as_ref())
                })
    ...
    for dir in config_dirs
        .iter()
        .chain(std::iter::once(&global_config_dir))
    {
        ...
        if variables_path.exists() {
            match fs::read_to_string(&variables_path) {
                ...
                Err(err) => {
                    tracing::error!(
                        "Failed to read variables.yaml in dir {}: {:?}",
                        dir.display(),
                        err
                    );
                }
    ...
    .await
    {
        Ok(the_get) => {
            ...
            tracing::info!(
                "schema_struct {}",
                serde_json::to_string_pretty(&schema_struct).unwrap()
            );
            tracing::info!(
                "sample values {}",
                serde_json::to_string_pretty(&the_get.integr_values).unwrap()
            );
            let mut msg = format!(
            ...
            if the_get.integr_config_exists {
                ...
                if let serde_yaml::Value::Mapping(ref mut map) = yaml_value {
                    ...
                    available_map.insert(
                        serde_yaml::Value::String("on_your_laptop".to_string()),
                        serde_yaml::Value::Bool(schema_struct.available.on_your_laptop_possible),
                    );
                    available_map.insert(
                        serde_yaml::Value::String("when_isolated".to_string()),
                        serde_yaml::Value::Bool(schema_struct.available.when_isolated_possible),
                    );
                    map.insert(
                        serde_yaml::Value::String("available".to_string()),
                        serde_yaml::Value::Mapping(available_map),
                    );
                }
    ...
    if messages.len() == 1 {
        ...
        tracing::error!(
            "more than 1 message when mixing configurtion chat context, bad things might happen!"
        );
    }

    messages.splice(
        0..0,
        vec![system_message, context_file_message, schema_message],
    );
