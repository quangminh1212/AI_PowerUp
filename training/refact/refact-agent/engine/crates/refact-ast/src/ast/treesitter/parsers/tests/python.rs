...
mod tests {
    ...
    use crate::ast::treesitter::parsers::tests::{
        base_declaration_formatter_test, base_parser_test, base_skeletonizer_test,
    };
    ...
    #[ignore]
    fn parser_test() {
        let mut parser: Box<dyn AstLanguageParser> =
            Box::new(PythonParser::new().expect("PythonParser::new"));
    ...
    fn skeletonizer_test() {
        ...
        let file = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join("src/ast/treesitter/parsers/tests/cases/python/calculator.py");
        assert!(file.exists());

        base_skeletonizer_test(
            &LanguageId::Python,
            &mut parser,
            &file,
            CALCULATOR_PY_CODE,
            CALCULATOR_PY_SKELETON,
        );
    }
