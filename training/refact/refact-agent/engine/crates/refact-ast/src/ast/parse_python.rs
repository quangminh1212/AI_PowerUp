...
use crate::ast::parse_common::{
    ContextAnyParser, Thing, any_child_of_type, type_deindex, type_deindex_n, type_call,
    type_zerolevel_comma_split,
};
...
fn py_add_a_thing<'a>(
    ...
    if let Some(thing_exists) = cx.ap.things.get(thing_path) {
        if thing_exists.thing_kind != thing_kind {
            let msg = cx.ap.error_report(
                node,
                format!(
                    "py_add_a_thing both {:?} and {:?} exist",
                    thing_exists.thing_kind, thing_kind
                ),
            );
            debug!(cx, "{}", msg);
            return (false, type_new.clone());
        }
        let good_idea_to_write =
            type_problems(&thing_exists.type_resolved) > type_problems(&type_new);
        if good_idea_to_write {
            debug!(
                cx,
                "TYPE UPDATE {thing_kind} {thing_path} TYPE {} problems={:?} => {} problems={:?}",
                thing_exists.type_resolved,
                type_problems(&thing_exists.type_resolved),
                type_new,
                type_problems(&type_new)
            );
...
fn py_resolve_dotted_creating_usages<'a>(
    cx: &mut ContextPy,
    node: &Node<'a>,
    path: &Vec<String>,
    allow_creation: bool,
    ...
    match node.kind() {
        ...
        _ => {
            let msg = cx
                .ap
                .error_report(node, format!("py_resolve_dotted_creating_usages syntax"));
            debug!(cx, "{}", msg);
        }
    }
    None
            ...
            for j in 0..left.child_count() {
                ...
                match child.kind() {
                    ...
                    _ => {
                        let msg = cx
                            .ap
                            .error_report(&child, format!("py_lhs_tuple list syntax"));
                        debug!(cx, "{}", msg);
                    }
                }
            }
...
fn py_assignment<'a>(cx: &mut ContextPy, node: &Node<'a>, path: &Vec<String>, is_for_loop: bool) {
    ...
    let (lhs_tuple, is_list) = py_lhs_tuple(
        cx,
        &left_node.unwrap(),
    ...
    for n in 0..lhs_tuple.len() {
        ...
        if is_list {
            py_var_add(
                ...
                lhs_lvalue,
                lvalue_type.clone(),
                type_deindex_n(rhs_type.clone(), n),
                path,
            );
        } else {
...
fn py_var_add<'a>(
    ...
    let lvalue_usage =
        if let Some(u) = py_resolve_dotted_creating_usages(cx, lhs_lvalue, path, true) {
            u
        } else {
            let msg = cx
                .ap
                .error_report(lhs_lvalue, format!("py_var_add cannot form lvalue"));
            debug!(cx, "{}", msg);
            return;
        };
    let lvalue_path;
    if lvalue_usage.targets_for_guesswork.is_empty() {
        // no guessing, exact location
        ...
        let msg = cx
            .ap
            .error_report(lhs_lvalue, format!("py_var_add cannot create"));
        debug!(cx, "{}", msg);
        return;
    }
    let potential_new_type = if type_problems(&lvalue_type) > type_problems(&rhs_type) {
        rhs_type.clone()
    } else {
        lvalue_type.clone()
    };
    let (upd, best_return_type) =
        ...
        "generic_type" => {
            ...
            for i in 0..node.child_count() {
                ...
                match (child.kind(), child_text.as_str()) {
                    ...
                    ("identifier", _) | ("attribute", _) => {
                        inside_type = format!("ERR/ID/{}", child_text)
                    }
                    ("type_parameter", _) => {
                        inside_type = py_type_generic(cx, Some(child), path, level + 1)
                    }
                ...
                comma_sep_types.push_str(
                    match child.kind() {
                        ...
                        _ => format!(
                            "SOMETHING/{:?}/{}",
                            child.kind(),
                            cx.ap.code[child.byte_range()].to_string()
                        ),
                    }
                    .as_str(),
                );
...
fn py_string<'a>(cx: &mut ContextPy, node: &Node<'a>, path: &Vec<String>) -> String {
    for i in 0..node.child_count() {
        ...
        match child.kind() {
            "interpolation" => {
                let _ = py_type_of_expr_creating_usages(
                    cx,
...
fn py_type_of_expr_creating_usages<'a>(
    cx: &mut ContextPy,
    node: Option<Node<'a>>,
    path: &Vec<String>,
) -> String {
    ...
    let type_of = match node.kind() {
        ...
        "binary_operator" => {
            let left_type =
                py_type_of_expr_creating_usages(cx, node.child_by_field_name("left"), path);
            let _right_type =
                py_type_of_expr_creating_usages(cx, node.child_by_field_name("right"), path);
            let _op =
                cx.ap.code[node.child_by_field_name("operator").unwrap().byte_range()].to_string();
            left_type
        }
        "unary_operator" | "not_operator" => {
            // ignore "operator"
            let arg_type =
                py_type_of_expr_creating_usages(cx, node.child_by_field_name("argument"), path);
            arg_type
        }
        ...
        "call" => {
            ...
            let arg_types =
                py_type_of_expr_creating_usages(cx, node.child_by_field_name("arguments"), path);
            let ret_type = type_call(ftype.clone(), arg_types.clone());
            ret_type
        }
        "identifier" | "dotted_name" | "attribute" => {
            let dotted_type =
                if let Some(u) = py_resolve_dotted_creating_usages(cx, &node, path, false) {
                    if u.resolved_as.starts_with("!") {
                        // trivial function, like "range" that has type ![int]
                        u.resolved_as
        ...
        "subscript" => {
            let typeof_value =
                py_type_of_expr_creating_usages(cx, node.child_by_field_name("value"), path);
            ...
            "argument_list" => {
                for j in 0..child.child_count() {
                    ...
                    match arg.kind() {
                        "identifier" | "attribute" => {
                            if let Some(a_type) =
                                py_resolve_dotted_creating_usages(cx, &arg, path, false)
                            {
                                if !a_type.resolved_as.is_empty() {
                                    // XXX losing information, we have resolved usage, turning it into approx 🔎-link
                                    let after_last_colon_colon =
                                        a_type.resolved_as.split("::").last().unwrap().to_string();
                                    ...
                                    let after_last_colon_colon = a_type
                                        .targets_for_guesswork
                                        .first()
                                        .unwrap()
                                        .split("::")
                                        .last()
...
fn py_function<'a>(cx: &mut ContextPy, node: &Node<'a>, path: &Vec<String>) {
    ...
    for i in 0..params.child_count() {
        ...
        match param_node.kind() {
            ...
            "typed_parameter" | "typed_default_parameter" | "default_parameter" => {
                ...
                type_resolved =
                    py_type_generic(cx, param_node.child_by_field_name("type"), &func_path, 0);
                let _defvalue_type = py_type_of_expr_creating_usages(
                    cx,
                    param_node.child_by_field_name("value"),
                    &func_path,
                );
            }
            "," | "(" | ")" => continue,
            ...
            _ => {
                let msg = cx
                    .ap
                    .error_report(&param_node, format!("py_function parameter syntax"));
                debug!(cx, "{}", msg);
                continue;
            }
        }
        if param_name.is_empty() {
            let msg = cx
                .ap
                .error_report(&param_node, format!("py_function nameless param"));
            debug!(cx, "{}", msg);
            continue;
        }
    ...
    let (upd2, best_return_type) =
        py_add_a_thing(cx, &func_path_str, 'f', format!("!{}", ret_type), node);
    if upd1 || upd2 {
        cx.ap.defs.insert(
            func_path_str,
            AstDefinition {
    ...
    match node.kind() {
        ...
        | "elif_clause" => {
            for i in 0..node.child_count() {
                let child = node.child(i).unwrap();
                match child.kind() {
                    "if" | "elif" | "else" | ":" | "integer" | "float" | "string" | "false"
                    | "true" => {
                        continue;
                    }
                    "return_statement" => {
                        ret_type = py_type_of_expr_creating_usages(cx, child.child(1), path);
                    }
                    _ => {
                        let _ = py_body(cx, &child, path);
                    }
                }
            }
        }
            ...
            if let Some(definition) = node.child_by_field_name("definition") {
                match definition.kind() {
                    ...
                    _ => {
                        let msg = cx.ap.error_report(
                            &definition,
                            format!("decorated_definition with unknown definition type"),
                        );
                        debug!(cx, "{}", msg);
                    }
                }
        ...
        "while_statement" => {
            let _cond_type =
                py_type_of_expr_creating_usages(cx, node.child_by_field_name("condition"), path);
            let _body_type = py_body(cx, &node.child_by_field_name("body").unwrap(), path);
        }
        "call" | "comparison_operator" => {
...
mod tests {
    ...
    fn test_parse_py_tort1() {
        ...
        let annotated = py_parse4test(code);
        std::fs::write(
            "src/ast/alt_testsuite/py_torture1_attr_annotated.py",
            annotated,
        )
        .expect("Unable to write file");
    }
    ...
    fn test_parse_py_tort2() {
        ...
        let annotated = py_parse4test(code);
        std::fs::write(
            "src/ast/alt_testsuite/py_torture2_resolving_annotated.py",
            annotated,
        )
        .expect("Unable to write file");
    }
    ...
    fn test_parse_py_goat_library() {
        ...
        let annotated = py_parse4test(code);
        std::fs::write(
            "src/ast/alt_testsuite/py_goat_library_annotated.py",
            annotated,
        )
        .expect("Unable to write file");
    }
    ...
    fn test_parse_py_goat_main() {
        ...
        let annotated = py_parse4test(code);
        std::fs::write("src/ast/alt_testsuite/py_goat_main_annotated.py", annotated)
            .expect("Unable to write file");
    }
}
