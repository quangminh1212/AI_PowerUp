    ...
    fn build_runtime(&self) -> Result<ProviderRuntime, String> {
        ...
        if let Some(num_ctx) = self.num_ctx {
            extra_headers.insert(
                refact_llm::adapters::ollama::OLLAMA_NUM_CTX_HEADER.to_string(),
                num_ctx.to_string(),
            );
        }
        if let Some(keep_alive) = self.keep_alive.as_deref().filter(|v| !v.trim().is_empty()) {
            extra_headers.insert(
                refact_llm::adapters::ollama::OLLAMA_KEEP_ALIVE_HEADER.to_string(),
...
mod tests {
    ...
    fn ollama_runtime_stores_options_as_internal_headers() {
        ...
        assert_eq!(
            runtime
                .extra_headers
                .get(refact_llm::adapters::ollama::OLLAMA_NUM_CTX_HEADER)
                .map(String::as_str),
            Some("65536")
        );
        assert_eq!(
            runtime
                .extra_headers
                .get(refact_llm::adapters::ollama::OLLAMA_KEEP_ALIVE_HEADER)
                .map(String::as_str),
            Some("30m")
        );
