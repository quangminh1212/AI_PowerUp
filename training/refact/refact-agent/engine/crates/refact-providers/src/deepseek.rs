...
#[async_trait]
impl ProviderTrait for DeepseekProvider {
    fn name(&self) -> &str {
        "deepseek"
    }
...
fields:
  api_key:
    f_type: string_long
    f_desc: "DeepSeek API key from platform.deepseek.com"
    f_placeholder: "sk-..."
    f_label: "API Key"
    smartlinks:
      - sl_label: "Get API Key"
        sl_goto: "https://platform.deepseek.com/api_keys"
description: |
  DeepSeek AI models (DeepSeek Coder, DeepSeek Chat).
available:
...
"#
    ...
    fn build_runtime(&self) -> Result<ProviderRuntime, String> {
        let api_key = resolve_env_var(&self.api_key, "", "deepseek api_key");
    ...
    fn has_credentials(&self) -> bool {
        let key = resolve_env_var(&self.api_key, "", "deepseek api_key");
        !key.is_empty()
    }
