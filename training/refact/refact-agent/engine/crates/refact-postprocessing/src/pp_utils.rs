...
pub async fn pp_resolve_ctx_file_paths(
    ...
    for context_file in context_file_vec.iter_mut() {
        ...
        let candidates = ctx.correct_to_nearest_filename(&path_as_presented, 5).await;
        let cpath = match candidates.first() {
            Some(c) => ctx.canonical_path(c),
            None => ctx.canonical_path(&path_as_presented),
        };
    ...
    let shortified_vec: Vec<String> = ctx.shortify_paths(&unique_cpaths_vec).await;
    unique_cpaths_vec
        .into_iter()
...
pub async fn pp_ast_markup_files(
    ...
    for (cpath, short) in pp_resolve_ctx_file_paths(ctx.clone(), context_file_vec).await {
        ...
        let text = match ctx.read_file(&cpath_pathbuf).await {
            Ok(text) => text,
            Err(e) => {
        ...
        let defs = ctx.doc_defs_for_path(&cpath).await;
        let mut symbols_sorted_by_path_len = defs;
...
pub async fn context_msgs_from_paths(
    ...
    for file_name in files_set {
        let path = ctx.canonical_path(&file_name);
        let text = match ctx.read_file(&path).await {
            Ok(text) => text,
            Err(e) => {
