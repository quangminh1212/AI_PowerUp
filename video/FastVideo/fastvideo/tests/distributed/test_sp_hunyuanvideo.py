...
def _build_tiny_hunyuanvideo_config() -> HunyuanVideoConfig:
    arch_config = HunyuanVideoArchConfig(
        patch_size=1,
        patch_size_t=1,
        in_channels=4,
        out_channels=4,
    ...
    return HunyuanVideoConfig(arch_config=arch_config)
...
def _run_worker(mode: str, output_path: Path) -> None:
    ...
    try:
        ...
        config = _build_tiny_hunyuanvideo_config()
        model = HunyuanVideoTransformer3DModel(config=config, hf_config={})
...
# pytest -sv fastvideo/tests/distributed/test_sp_hunyuanvideo.py
