...
from transformers.utils import TransformersKwargs, is_torch_available
...
class GlmAsrForConditionalGeneration(GlmAsrPreTrainedModel, GenerationMixin):
    ...
    def prepare_inputs_for_generation(self, *args, is_first_iteration: bool = False, **kwargs):
        ...
        # 4.57's GenerationMixin doesn't pass ``is_first_iteration``,
        # so we infer it from the absence of cached KVs. Without this,
        # ``input_features`` would never be forwarded into ``model.forward``
        # and the audio embeddings would silently never reach the LM.
        if not is_first_iteration:
            past = kwargs.get("past_key_values") or (args[1] if len(args) >= 2 else None)
            if past is None:
                is_first_iteration = True
            else:
                seq_len = getattr(past, "get_seq_length", lambda: 0)()
                if seq_len == 0:
                    is_first_iteration = True
