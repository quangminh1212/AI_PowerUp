...
class GlmAsrProcessor(ProcessorMixin):
    # 5.x's ProcessorMixin auto-discovers sub-attribute classes from the
    # config; 4.57 needs them as class attributes. ``tokenizer_class``
    # in the upstream repo is ``TokenizersBackend`` (5.x-only); the
    # generic ``PreTrainedTokenizerFast`` loads the raw tokenizer.json
    # the repo actually ships and gives us identical behaviour.
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, **kwargs):
        """4.57-compatible loader.

        Bypasses ``ProcessorMixin._get_arguments_from_pretrained`` so we
        can pass per-class kwargs separately:

        - ``WhisperFeatureExtractor`` is loaded as upstream expects.
        - ``PreTrainedTokenizerFast`` is loaded with
          ``extra_special_tokens={}`` because the upstream
          ``tokenizer_config.json`` ships ``extra_special_tokens`` as a
          *list* (5.x convention) but 4.57's tokenizer init expects a
          *dict*. The actual special tokens are preserved in
          ``tokenizer.json`` regardless.
        """
        import json
        import os
        ...
        def _read(name: str) -> str | None:
            if os.path.isdir(pretrained_model_name_or_path):
                fp = os.path.join(pretrained_model_name_or_path, name)
            else:
                try:
                    fp = hf_hub_download(pretrained_model_name_or_path, name)
                except Exception:
                    return None
            return open(fp).read() if os.path.exists(fp) else None

        proc_text = _read("processor_config.json") or "{}"
        proc_config = json.loads(proc_text)
        chat_template = _read("chat_template.jinja")

        return cls(
            feature_extractor=feature_extractor,
            tokenizer=tokenizer,
            chat_template=chat_template,
            audio_token=proc_config.get("audio_token", "<|pad|>"),
            default_transcription_prompt=proc_config.get("default_transcription_prompt",
                                                         "Please transcribe this audio into text"),
            max_audio_len=proc_config.get("max_audio_len", 655),
        )
    ...
    def decode(self, *args, strip_prefix=False, **kwargs):
        """Decode a single sequence of token ids, optionally stripping the assistant framing.

        4.57 fix: route to ``tokenizer.decode`` for a single sequence
        and ``tokenizer.batch_decode`` for a batched input. Upstream
        relied on 5.x's ``tokenizer.decode`` accepting batched input
        directly; 4.57's does not.
        """
        decoded = self.tokenizer.decode(*args, **kwargs)
        if strip_prefix:
            decoded = self._strip_assistant_prefix_and_quotes(decoded)
        return decoded
