<!-- source: https://github.com/shashankg-dev404/react-native-gemma-agent.git sha: 0ed64a86a2247c69a686ec5f1c9953a5d71402bd readme: main/README.md -->
# shashankg-dev404/react-native-gemma-agent

React Native SDK for on-device AI agents powered by Google Gemma 4. Run LLM inference locally with function calling, tool use, and a pluggable JS skill system — no server, no API key, 100% private. Built on llama.rn + GGUF. Supports agent loops, streaming, skills via WebView sandbox. Android first.

---

# react-native-gemma-agent

[![npm version](https://img.shields.io/npm/v/react-native-gemma-agent.svg)](https://www.npmjs.com/package/react-native-gemma-agent)
[![CI](https://github.com/shashankg-dev404/react-native-gemma-agent/actions/workflows/ci.yml/badge.svg)](https://github.com/shashankg-dev404/react-native-gemma-agent/actions/workflows/ci.yml)
[![License: MIT](https://img.shields.io/npm/l/react-native-gemma-agent.svg)](LICENSE)
![Android API 26+](https://img.shields.io/badge/Android-API%2026%2B-brightgreen.svg)
![React Native 0.76+](https://img.shields.io/badge/React%20Native-0.76%2B-blue.svg)
...
- 📝 Full responses per turn today; live token-by-token streaming is next up (see [Roadmap](#future-plans))
...
`generateStructured()` gets JSON out of the model that matches a schema. There's no native grammar-constrained decoding underneath it (see [ADR-009](docs/ADR/009-structured-output.md) for why): the schema is injected into the system prompt, the response is parsed and validated, and a failed attempt gets up to `maxRetries` more tries with the validation error fed back to the model.

```tsx
import { generateStructured } from 'react-native-gemma-agent';
import { z } from 'zod';

const { object, attempts } = await generateStructured(engine, {
  schema: z.object({ title: z.string(), date: z.string() }),
  prompt: 'Dinner with Priya on Saturday 8pm',
});
```

Accepts a Zod v4 schema (uses its built-in `schema.toJSONSchema()`), a plain JSON Schema object, or a Zod v3 schema you've converted yourself with [`zod-to-json-schema`](https://www.npmjs.com/package/zod-to-json-schema) — `toJsonSchema()` throws with that suggestion if it gets a v3 schema directly, since the SDK doesn't depend on that package itself. The same primitive backs `generateObject()` on the AI SDK provider above.
...
Hook for model download management. The model is `~3.1 GB`. An interrupted download restarts from zero on the next `download()` call (no resume). For a dev loop that avoids re-downloading on every rebuild, use `npx react-native-gemma-agent pull <model-id>` once on your dev machine and `adb push` the cached file to `/data/local/tmp/`.
...
- [ ] Live token-by-token streaming (responses currently render as a full block per turn; this is the top priority for the next release)
